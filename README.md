# DEEPBLOOM :: Autonomous Shell Orchestrator v4.0

This agent, DEEPBLOOM, is a surgical bash orchestrator designed to execute commands, not conversations. Its core philosophy is 'Precision Through Action', ensuring every step taken is verifiable and purposeful.

## Key Principles:
-   **Inviolable Laws:** Adheres strictly to a set of laws ensuring output consistency, stateless execution, TTY logging, verification of changes, and user consent.
-   **Structured Workflow:** Follows a clear, example-driven workflow for task completion, from establishing context to final verification and logging.
-   **Git & GitHub Integration:** Employs disciplined branch strategy, commit discipline, and PR workflow for seamless collaboration and version control.
-   **Operational Matrix:** A defined set of commands and their associated verification steps for common operations.

## Features:
-   **Autonomous Execution:** Capable of executing multi-step tasks in a bash environment.
-   **Self-Correction:** Designed to verify actions and adapt to unexpected outcomes.
-   **User Interaction:** Utilizes zenity for crucial user confirmations (e.g., destructive actions).
-   **Detailed Logging:** Maintains a master-dev-log.md for persistent task history.
-   **Performance Feedback:** Integrates with llm feedback for continuous model improvement.

## Inspiration and Metaprompting Insights (from jina-metaprompt.md):
...
A note about the new version: it integrates embedding models directly with the llm embed API, offering powerful capabilities for semantic search and data analysis.

## How to Use:
(Placeholder for usage instructions, typically includes installation, configuration, and example commands.)
